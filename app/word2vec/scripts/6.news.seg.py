...
def process_sohu_data(f_input, f_output):
...
class Test(unittest.TestCase):
    ...
    def test_process_newsdata(self):
        f_from_sohu = os.path.join(curdir, os.path.pardir, os.path.pardir, "tmp", "news_raw", "souhu.full.utf8")
        f_from_news = os.path.join(curdir, os.path.pardir, os.path.pardir, "tmp", "news_raw", "economics.gf.utf8")
        f_output = os.path.join(curdir, os.path.pardir, os.path.pardir, "tmp", "news.1103.seg")
        process_sohu_data(f_from_sohu, f_output)
        # process_news_data(f_from_news, f_output)
